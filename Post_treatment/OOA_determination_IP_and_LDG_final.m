...
new_resultIP=zeros(Nx-22*4,Ny+2*N5);
...
for n=1:1:N5%Loop over tau
    for k=1:1:N3%loop over c
        for m=1:1:2
            OOA_arrayIP=resultIP(2+m+2*(k-1),3+N2+(N2+(N2-1)+1)*(n-1)+N2-1-1);
            OOA_meanIP=mean(OOA_arrayIP);
            OOA_polyIP=OOA_arrayIP;
        ...
        for m=1:1:N4%loop over kappa
            %For LDG
            OOA_arrayLDG=resultLDG(2+m+N4*(k-1),3+N2+(N2+(N2-1)+1)*(n-1)+N2-1-1);
            OOA_meanLDG=mean(OOA_arrayLDG);
            OOA_polyLDG=OOA_arrayLDG;
                    ...
                    otherwise 
                        switch k
                            case 1
                                ...
                                OOA_arrayIP=new_resultIP(3+2*(i-1),3+(N2+(N2-1)+1)*(k-1)+N2+(N2-1)-1);
                            ...
                            case 2
                                ...
                                OOA_arrayIP=new_resultIP(3+2*(i-1),3+3+(N2+(N2-1)+1)*(k-1)+N2+(N2-1)+1-3);
